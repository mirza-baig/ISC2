...
interface VolunteerSearchHitProps {
...
type VolunteerSearchItemProps = {
  label: string;
  value?: string;
  tooltip?: string | number;
};

const VolunteerSearchItem = ({ label, value, tooltip }: VolunteerSearchItemProps) => {
  ...
  return (
    <li className="flex flex-row items-center md:items-baseline">
      <span className="eyebrow sm:text-right w-36">{label}</span>
...
const VolunteerSearchHit = ({ hit, labels }: VolunteerSearchHitProps) => {
  ...
  const ctaLabel = useMemo(() => {
    if (hit.url) {
      return hit.cta || hit.buttonLabel || 'Learn more';
    }

    return '';
  }, [hit.url, hit.cta, hit.buttonLabel]);
    ...
    >
      <div className="flex flex-col sm:flex-row w-full sm:space-y-0">
        ...
        <ul className="w-full sm:w-auto my-6 sm:my-0 sm:ml-16 space-y-4 sm:space-y-3 md:w-96">
          ...
          />
          <VolunteerSearchItem label={labels.trainingMethodLabel} value={hit.category} />
          <VolunteerSearchItem
          ...
          />
          <VolunteerSearchItem label={labels.locationLabel} value={hit.contact} />
          <VolunteerSearchItem label={labels.buttonLabel} value={hit.membershipRequired} />
        </ul>
      </div>
...
export default VolunteerSearchHit;
