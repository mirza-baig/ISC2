...
import {
  rewriteLayoutServiceMediaUrls,
  shouldRewriteLayoutMediaForPreviewEnv,
} from 'lib/utils/rewrite-sitecore-media-urls';
import { Plugin } from '..';
...
class PreviewModePlugin implements Plugin {
  ...
  async exec(props: SitecorePageProps, context: GetServerSidePropsContext | GetStaticPropsContext) {
    ...
    if (isEditingMetadataPreviewData(context.previewData)) {
      ...
      props.layoutData = shouldRewriteLayoutMediaForPreviewEnv()
        ? rewriteLayoutServiceMediaUrls(data.layoutData)
        : data.layoutData;
