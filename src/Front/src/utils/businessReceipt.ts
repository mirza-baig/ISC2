...
/**
 * Attribute names checked for the in-person class venue, in order. None is confirmed
 * against commercetools — no location attribute appears in LINE_ITEMS_ATTRIBUTES or
 * anywhere else in this repo — so the list is deliberately wide and the row is hidden
 * when nothing matches. Trim it once the real attribute name is known.
 */
