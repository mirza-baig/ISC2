...
const SearchBox = ({ hideInput, filterLabel, resultsFoundLabel }: SearchBoxProps) => {
  ...
  return (
    <div className="flex flex-col sm:flx-row sticky -top-px sm:relative z-50" ref={ref}>
      ...
      {Boolean(results?.nbHits) && (
        <div
          className={clsx(
            'w-full py-6 px-5 bg-white-00 sm:p-0 sm:pb-1 flex flex-row justify-between items-center shadow-search-filters-pinned sm:flex-col sm:items-start sm:hidden sm:shadow-none',
        ...
        >
          ...
          <SearchOpenFiltersMenu filterLabel={filterLabel} openMenu={openFiltersMenu} />
        </div>
      )}
    </div>
  );
