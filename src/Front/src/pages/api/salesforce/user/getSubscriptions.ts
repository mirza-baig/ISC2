...
export default async function getSubscriptions(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const response = await fetch(
      `${process.env.SALESFORCE_CLOUDHUB_URL}/v1/user/subscriptions?externalID=${externalID}&email=${email}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok || data?.error) {
      ...
      throw 'Error while fetching subscriptions. See server logs for more information.';
    }
